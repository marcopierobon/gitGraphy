      ...
      return new Promise((resolve, reject) => {
        exec(
          ...
          (err: any, stdout: any, stderr: any) => {
            ...
            const fileNameAndSizePairs: [string] = lines.map((line: string) => {
...
//const powershellCmd = `cd '${dir}'; git ls-tree -r -l --abbrev --full-name HEAD | Foreach {($_ -split '\\s+',4)[3,4]} |Sort-Object -Descending {[int]($_ -split "\\s+")[0]}  | Select-Object -Skip ${skipNumberOfFiles} -first 10 `;
      return new Promise((resolve, reject) => {
        exec(
          ...
          (err: any, stdout: any, stderr: any) => {
            ...
            FilesSizeRetriever.filesWithSizesArray = fileNameAndSizePairs;
            var lowestBoundary = (skipNumberOfFiles < FilesSizeRetriever.filesWithSizesArray.length) ? skipNumberOfFiles : FilesSizeRetriever.filesWithSizesArray.length;
            var upperBoundary = Math.min(FilesSizeRetriever.filesWithSizesArray.length, (10 + skipNumberOfFiles));
            return resolve(FilesSizeRetriever.filesWithSizesArray
              .slice(lowestBoundary, upperBoundary));
          }
        );
      });
    }
  ...
  return new Promise<string[]>((resolve) => {
    ...
    resolve(FilesSizeRetriever.filesWithSizesArray
      .slice(lowestBoundary, upperBoundary));
  });
  }
  ...
  private static determineRightSizeSpecifier(fileSizeInBytes: string): string{
    ...
    return (Math.round(parsedNumber * 100) / 100).toFixed(2) + fileSizeSpecifier;
  }
}
